...
std::vector<BlockData> NodeClient::get_blockchain(uint64_t start_index) {
  ...
  if (!status.ok()) {
    std::cout << "[Client] GetBlockchain RPC failed: "
  ...
  for (const auto& proto_block : response.blocks()) {
    ...
    for (const auto& proto_tx : proto_block.transactions()) {
      ...
      tx.recipient_public_key = std::string(
          proto_tx.recipient_public_key().begin(),
  ...
  std::cout << "[Client] Received " << blocks.size()
            << " blocks from " << target_address_ << std::endl;
...
bool NodeClient::get_transaction_status(const std::string& transaction_id,
  ...
  if (!status.ok()) {
    std::cout << "[Client] GetTransactionStatus RPC failed: "
              << status.error_message() << std::endl;
    return false;
  }

  block_hash = std::string(response.block_hash().begin(),
