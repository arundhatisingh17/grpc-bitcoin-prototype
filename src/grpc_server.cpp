...
{
    // Convert protobuf Transaction to our C++ struct
    GeneratedTransaction tx = proto_to_transaction(request->transaction());

    // Try to add it to this node's mempool
...
{
    // Convert protobuf Block to our C++ struct
    BlockData block = proto_to_block(request->block());

    // Validate and add to this node's chain
    ...
    bitcoin::GetBlockchainResponse* response)
{
    ...
    // Send blocks starting from the requested index
...
{
    std::string tx_id(request->transaction_id().begin(), request->transaction_id().end());

    // Search through the chain for this transaction
    std::vector<BlockData> chain = node_.get_chain();
    ...
    // Not found in any block — still unconfirmed
...
GeneratedTransaction NodeServiceImpl::proto_to_transaction(const bitcoin::Transaction& proto_tx) {
    ...
    tx.is_coinbase = tx.sender_public_key.empty();  // no sender = coinbase
    return tx;
}

bitcoin::Transaction NodeServiceImpl::transaction_to_proto(const GeneratedTransaction& tx) {
...
void run_grpc_server(Node& node, const std::string& address) {
    ...
    std::cout << "[Server " << node.get_node_id() << "] Listening on " << address << std::endl;
