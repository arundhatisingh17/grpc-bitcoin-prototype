...
Node::Node(const std::string& node_id, const std::string& address, uint32_t initial_difficulty)
...
bool Node::add_transaction_to_mempool(const GeneratedTransaction& tx) {
    // Basic check: don't add duplicates (by timestamp + keys as a simple check)
...
std::vector<GeneratedTransaction> Node::get_mempool() const {
    return mempool_;
}
...
bool Node::mine_block() {
    if (mempool_.empty()) {
        std::cout << "[Node " << node_id_ << "] Mempool empty, nothing to mine" << std::endl;
        return false;
    }
    ...
    for (const auto& tx : mempool_) {
        std::string tx_data = tx.sender_public_key + tx.recipient_public_key +
                              std::to_string(tx.amount) + std::to_string(tx.timestamp);
        new_block.transaction_ids.push_back(sha256(tx_data));
    }

    // --- Proof of Work loop ---
    // Keep incrementing the nonce until the block hash meets the difficulty target
    std::cout << "[Node " << node_id_ << "] Mining block #" << chain_.size()
              << " (difficulty: " << new_block.difficulty_target
              << ", transactions: " << new_block.transactions.size() << ")" << std::endl;
...
std::vector<BlockData> Node::get_chain() const {
    return chain_;
}

BlockData Node::get_latest_block() const {
    return chain_.back();
}

size_t Node::get_chain_length() const {
    return chain_.size();
}

bool Node::add_block(const BlockData& block) {
    if (!validate_block(block)) {
        std::cout << "[Node " << node_id_ << "] Rejected invalid block" << std::endl;
        return false;
    }

    chain_.push_back(block);
    std::cout << "[Node " << node_id_ << "] Accepted block #" << (chain_.size() - 1) << std::endl;
    return true;
}
...
bool Node::validate_block(const BlockData& block) const {
    ...
    if (block.prev_block_hash != expected_prev_hash) {
        std::cout << "[Node " << node_id_ << "] Block rejected: prev_hash mismatch" << std::endl;
        return false;
    }
    ...
    if (block.merkle_root != expected_merkle) {
        std::cout << "[Node " << node_id_ << "] Block rejected: merkle root mismatch" << std::endl;
        return false;
    }
    ...
    if (!meets_difficulty(block_hash, block.difficulty_target)) {
        std::cout << "[Node " << node_id_ << "] Block rejected: PoW invalid" << std::endl;
        return false;
    }
...
void Node::add_peer(const PeerInfo& peer) {
    peers_.push_back(peer);
}

std::vector<PeerInfo> Node::get_peers() const {
    return peers_;
}
...
BlockData Node::create_genesis_block() {
    ...
    genesis.merkle_root = "";
    genesis.timestamp = 1231006505;  // January 3, 2009 — same as Bitcoin's genesis block
...
std::string Node::hash_block(const BlockData& block) const {
    ...
    std::string header = block.prev_block_hash +
                         block.merkle_root +
...
std::string Node::compute_merkle_root(const std::vector<GeneratedTransaction>& transactions) const {
...
bool Node::meets_difficulty(const std::string& hash, uint32_t difficulty) const {
